...
- `scripts/fetch-transit-stations.py` queries Overpass for every Algeria `railway=tram_stop`, `station=subway`, `amenity=bus_station` and `highway=bus_stop` node (all cities: Algiers metro + Algiers/Oran/Constantine/Sidi Bel Abbès/Ouargla/Sétif/Mostaganem trams, plus bus terminals and stops nationwide). Each becomes a GeoJSON point with `mode` (`metro` / `tram` / `bus_station` / `bus`), `name`, `name_fr`, `name_ar`.
- The `tr_metro` / `tr_tram` / `tr_bus_station` / `tr_bus_stop` style layers **replace** the old base `poi_metro_station` / `poi_tram_stop` / `poi_bus_station` / `poi_bus_stop` layers, so there are no duplicate icons. Labels coalesce **Latin-first**: `name_fr` → `name` → `name_ar` → generic fallback (prefers readable French/Latin, falls back to Arabic, then a generic label).
- Metro & tram labels are forced (`text-allow-overlap`) so every station icon always carries a name. Bus stations and bus stops use collision binding (icon + label drop together) so dense areas declutter and you never get a naked icon.
...
**Approximate coverage** (regenerated each fetch, so counts drift as OSM grows): ~40 metro, ~356 tram, ~450 bus stations, ~4170 bus stops (~2700 of them named). Anything still showing a generic fallback ("Arrêt de bus", "Gare routière", "Station métro", "Arrêt de tram") genuinely has no name in OpenStreetMap — that is a data gap in OSM, not a styling bug, and the only fix is to map the name upstream or add it via `custom`.
